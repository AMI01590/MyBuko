...
export default function LandingPage() {
    ...
    return (
        <div className="min-h-screen bg-white text-slate-950 transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100">
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <Link href="#explore-preview" className="px-8 py-4 bg-white border-2 border-gray-200 text-gray-900 rounded-xl font-semibold hover:border-gray-300 transition-all flex items-center justify-center gap-2 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100 dark:hover:border-slate-500">
            ...
            {/* COMMUNITY ACCESS */}
            <section id="explore-preview" className="py-20 bg-white dark:bg-slate-950 transition-colors duration-300">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-12">
                        <p className="text-sm uppercase tracking-[0.28em] text-slate-500 dark:text-slate-400 font-semibold">Community access</p>
                        <h2 className="mt-4 text-4xl font-bold text-gray-900 dark:text-white">Explore community preview</h2>
                        <p className="mx-auto mt-4 max-w-2xl text-lg text-gray-600 dark:text-slate-300">
                            A locked preview of the Explore feed. The layout is visible, but the actions are translucent and unavailable until you sign in.
                        </p>
                    </div>

                    <div className="grid gap-6 lg:grid-cols-[1.4fr_0.6fr]">
                        <div className="relative overflow-hidden rounded-[2.5rem] border border-slate-200 bg-white/80 p-6 shadow-2xl shadow-slate-200/30 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/65 dark:shadow-slate-950/30">
                            <div className="absolute inset-0 bg-gradient-to-br from-white/85 via-white/40 to-slate-100/80 opacity-95 dark:from-slate-950/85 dark:via-slate-950/60 dark:to-slate-900/65"></div>
                            <div className="relative space-y-6">
                                <div className="flex items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-slate-100/80 px-4 py-3 text-slate-700 dark:border-slate-700 dark:bg-slate-800/70 dark:text-slate-200">
                                    <div>
                                        <p className="text-xs uppercase tracking-[0.3em] font-semibold">Explore feed</p>
                                        <p className="text-sm text-slate-500 dark:text-slate-400">Locked view</p>
                                    </div>
                                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-900/10 px-3 py-1 text-xs font-semibold text-slate-900 dark:bg-white/10 dark:text-white">
                                ...
                                <div className="space-y-4">
                                    {['Community stories', 'Trending goals', 'Follow activity'].map((label) => (
                                        <div key={label} className="rounded-3xl border border-slate-200 bg-slate-100/70 p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800/70">
                                            <div className="flex items-center justify-between gap-4">
                                                <p className="font-semibold text-slate-900 dark:text-white">{label}</p>
                                                <span className="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-slate-700 dark:text-slate-300">Preview</span>
                                            </div>
                                ...
                                <div className="rounded-3xl border border-slate-200 bg-slate-50/90 p-5 text-slate-700 dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-200">
                                    <div className="flex items-center gap-3 text-sm font-semibold">
                                        <Lock className="w-4 h-4" />
                                        <span>Sign in to unlock the full experience</span>
                                    </div>
                                    <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">This is the Explore community layout, but key actions remain blocked until you log in.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            {[
                                { title: 'Post a story', description: 'Create stories and share what you’re working on.' },
                                { title: 'Join goals', description: 'Participate in trending community goals.' },
                                { title: 'Follow peers', description: 'Comment, follow, and cheer on other members.' },
                            ].map((feature) => (
                                <div key={feature.title} className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-lg shadow-slate-200/20 backdrop-blur-xl dark:border-slate-700 dark:bg-slate-900/70 dark:shadow-slate-950/20">
                                    <div className="absolute inset-0 bg-gradient-to-br from-white/70 via-white/30 to-slate-100/70 opacity-90 dark:from-slate-900/70 dark:via-slate-900/50 dark:to-slate-950/80"></div>
                                    <div className="relative flex h-full flex-col gap-5">
                                        <div className="flex items-center gap-3">
                                            <div className="flex h-12 w-12 items-center justify-center rounded-3xl bg-slate-900/10 text-slate-900 dark:bg-white/10 dark:text-white">
                    ...
                    <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
                        {isLoggedIn ? (
                            <Link href="/explore" className="inline-flex items-center justify-center rounded-full bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700">
                                Go to Explore
                            </Link>
                        ) : (
                            <>
                                <Link href="/auth/login" className="inline-flex items-center justify-center rounded-full bg-blue-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition hover:bg-blue-700">
                                    Login to unlock
                                </Link>
                                <Link href="/auth/signup" className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:hover:bg-slate-900">
                                    Create an account
                                </Link>
                            </>
                        )}
